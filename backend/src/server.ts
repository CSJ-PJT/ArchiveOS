...
import { evaluateAuthenticatedReadRelay } from "./authenticatedReadRelay.js";
import {
  getTaskEvents,
...
async function relayArchiveOsAi(
  ...
  try {
    ...
    const integrationToken = request?.header?.("x-archiveos-integration-token");
    const readRelay = evaluateAuthenticatedReadRelay(path, request);
    if (readRelay.status) {
      response.status(readRelay.status).json({ error: readRelay.error });
      return;
    }
    if (readRelay.forward && readRelay.headers) {
      headers.set("Authorization", readRelay.headers.authorization);
      headers.set("X-Archive-Source-System", readRelay.headers.source);
      headers.set("X-Archive-Service-Scope", readRelay.headers.scope);
    }
    if (cookie) headers.set("cookie", cookie);
