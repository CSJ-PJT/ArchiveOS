...
function ExternalSystems({ data }: { data: AppData }) {
  ...
  return <div className="page-stack external-integration-view">
    ...
    <section className="external-list external-systems">
      ...
      {systems.filter((system) => system.systemId !== "archiveos").map((system) => <article key={system.systemId}><span className="eyebrow">REGISTERED SYSTEM</span><h2>{system.name}</h2><p>{system.role || system.statusReason || "ArchiveOS 관리 시스템"}</p><dl className="external-definition"><dt>상태</dt><dd><StatusBadge status={system.status}>{system.status}</StatusBadge></dd><dt>환경</dt><dd>{system.provider} · {system.environment}</dd><dt>대기 승인</dt><dd>{system.pendingApprovalCount.toLocaleString()}건</dd><dt>저장소</dt><dd>{system.repository || "등록 정보 없음"}</dd></dl></article>)}
    </section>
...
function capacityUtilization(summary: Record<string, unknown>, service: string) {
  ...
  const used = Number(readValue(summary, ["workforce.usedCapacity", "operations.workforce.usedCapacity", "runtimeWorkforce.usedCapacity"]));
  const effective = Number(readValue(summary, ["workforce.effectiveCapacity", "operations.workforce.effectiveCapacity", "runtimeWorkforce.effectiveCapacity"]));
...
function integrationConnectors(data: AppData) {
  ...
  for (const [target, status] of Object.entries(market)) values.push({ id: `market-${target}`, label: `Market → ${displayTarget(target)}`, enabled: !/blocked|disabled/i.test(String(status)), detail: `Market 운영 이벤트 계약 · ${String(status)}` });
  for (const [target, raw] of Object.entries(nexus)) { const item = raw && typeof raw === "object" ? raw as Record<string, unknown> : {}; values.push({ id: `nexus-${target}`, label: `Nexus → ${displayTarget(target)}`, enabled: item.enabled === true || /enabled|healthy/i.test(String(item.status ?? raw)), detail: `Nexus Outbox 라우팅 · ${String(item.status ?? raw ?? "상태 미수집")}` }); }
  if (Object.keys(logistics).length) values.push({ id: "logistics-ledger", label: "Logistics → Ledger", enabled: logistics.enabled === true || /enabled|healthy/i.test(String(logistics.status)), detail: `물류비 확정 계약 · ${String(logistics.status ?? "상태 미수집")}` });
...
function displayTarget(value: string) { if (/logit/i.test(value)) return "Logistics"; if (/ledger/i.test(value)) return "Ledger"; if (/nexus/i.test(value)) return "Nexus"; if (/archive/i.test(value)) return "ArchiveOS"; return value; }
