...
function ExternalSystems({ data }: { data: AppData }) {
  ...
  const atlasServices = data.atlas?.services ?? [];
  const atlasChecks = data.atlas?.recent_healthchecks ?? [];
  return <div className="page-stack external-integration-view">
    ...
    <section className="external-list external-systems">
      <article><span className="eyebrow">EXTERNAL / NON-CORE</span><h2>Atlas Platform</h2><p>Archive 핵심 서비스와 분리된 외부 프로젝트 묶음입니다. Atlas 상태는 핵심 생태계 상태를 바꾸지 않습니다.</p><dl className="external-definition"><dt>상태</dt><dd><StatusBadge status={data.atlas?.system?.current_status ?? "empty"}>{displayStatus(data.atlas?.system?.current_status)}</StatusBadge></dd><dt>등록 프로젝트</dt><dd>{atlasServices.length.toLocaleString()}개</dd><dt>마지막 확인</dt><dd>{latestAtlasCheck(atlasChecks)?.checked_at ? new Date(latestAtlasCheck(atlasChecks)!.checked_at).toLocaleString() : data.atlas?.system?.updated_at ? new Date(data.atlas.system.updated_at).toLocaleString() : "수집 없음"}</dd><dt>영향 범위</dt><dd>핵심 서비스 상태에 미포함</dd></dl></article>
      {atlasServices.map((service) => { const check = latestAtlasCheck(atlasChecks, service.service_id); const status = check?.status === "ok" ? "normal" : check?.status ? "degraded" : service.current_status; return <article key={`atlas-${service.service_id}`} className="atlas-project-card"><span className="eyebrow">ATLAS PROJECT</span><h2>{service.name}</h2><p>{service.note || `${service.service_type} 프로젝트의 읽기 전용 health 계약입니다.`}</p><dl className="external-definition"><dt>상태</dt><dd><StatusBadge status={status}>{displayStatus(status)}</StatusBadge></dd><dt>유형</dt><dd>{service.service_type} · {service.criticality}</dd><dt>마지막 확인</dt><dd>{check?.checked_at ? new Date(check.checked_at).toLocaleString() : "수집 대기"}</dd><dt>응답</dt><dd>{check?.http_status ? `HTTP ${check.http_status} · ${check.latency_ms ?? "-"}ms` : "최근 응답 없음"}</dd><dt>저장소</dt><dd>{service.repository || "등록 정보 없음"}</dd></dl></article>; })}
...
function integrationConnectors(data: AppData) {
  ...
  for (const service of data.atlas?.services ?? []) {
    const check = latestAtlasCheck(data.atlas?.recent_healthchecks ?? [], service.service_id);
    values.push({ id: `atlas-${service.service_id}`, label: `${service.name} → ArchiveOS`, enabled: service.enabled && check?.status === "ok", detail: check ? `Atlas 읽기 전용 health · ${displayStatus(check.status)} · HTTP ${check.http_status ?? "-"}` : "Atlas 읽기 전용 health · 첫 수집 대기" });
  }
  return values;
}
function latestAtlasCheck(checks: NonNullable<AppData["atlas"]>["recent_healthchecks"], serviceId?: string) { return checks.filter((check) => !serviceId || check.service_id === serviceId).slice().sort((left, right) => new Date(right.checked_at).getTime() - new Date(left.checked_at).getTime())[0]; }
...
function registeredSystemRole(systemId: string, fallback?: string | null) { return ({ "archive-market": "수요·주문·매출 원천", "archive-nexus": "제조·생산·출하 운영", "archive-logistics": "합성 물류·운송 운영", "archive-logitics": "합성 물류·운송 운영", "archive-ledger": "합성 거래·원장·정산 운영" } as Record<string, string>)[systemId] || fallback || "ArchiveOS 관리 시스템"; }
