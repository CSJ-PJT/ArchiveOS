...
function EcosystemFinanceView({ data }: { data: AppData }) {
  ...
  const waitingSettlement = data.liveFlow?.approvalBacklog ?? loadedWaiting;
  const approvalCountAvailable = data.liveFlow?.approvalBacklog != null || approvals.length > 0;
...
function ReconciliationView({ data }: { data: AppData }) {
  ...
  const nested = recordValue(ledger, ["reconciliation", "reconciliationSummary"]);
  const flat: Record<string, unknown> = {
    status: ledger.lastReconciliationStatus,
    reconciliationWarnings: ledger.reconciliationWarnings,
    reconciliationBacklog: (ledger.balance as Record<string, unknown> | undefined)?.reconciliationBacklog,
    settlementReady: ledger.settlementReady,
    settlementCompleted: ledger.settlementCompleted,
    lastSettlementAt: ledger.lastSettlementAt,
    transactionsReceived: ledger.transactionsReceived,
    transactionsProcessed: ledger.transactionsProcessed,
    callbackFailed: ledger.callbackFailed,
  };
  const reconciliation = nested ?? (Object.values(flat).some((value) => value !== null && value !== undefined) ? flat : null);
  const entries = reconciliation ? Object.entries(reconciliation).filter(([, value]) => value !== null && value !== undefined && typeof value !== "object").slice(0, 10) : [];
  const status = String(reconciliation?.status ?? reconciliation?.reconciliationStatus ?? "empty");
  return <section className="finance-table-card reconciliation-view"><header><div><span className="eyebrow">LEDGER RECONCILIATION</span><h2>대사 상태</h2><p>Ledger가 제공하는 합성 정산·대사 요약을 읽기 전용으로 표시합니다.</p></div><StatusBadge status={status}>{status === "empty" ? "데이터 없음" : status}</StatusBadge></header>{entries.length ? <div className="reconciliation-metric-grid">{entries.map(([key, value]) => <span key={key}><small>{humanize(key)}</small><strong>{String(value)}</strong></span>)}</div> : <div className="empty-copy">대사 요약 연동이 준비되지 않았거나 수집된 데이터가 없습니다.</div>}</section>;
}
