...
function MailDetail({ message, onReply }: { message: MailMessage | null; onReply: (address: string, subject: string) => void }) {
  ...
  return <article className="mail-detail"><header><StatusBadge status={message.direction === "inbound" ? "working" : "healthy"}>{message.direction === "inbound" ? "수신" : "발신"}</StatusBadge><h2>{message.subject || "(제목 없음)"}</h2><dl><dt>보낸 사람</dt><dd>{message.from_address}</dd><dt>받는 사람</dt><dd>{message.to_addresses.join(", ")}</dd>{message.cc_addresses.length ? <><dt>참조</dt><dd>{message.cc_addresses.join(", ")}</dd></> : null}<dt>일시</dt><dd>{new Date(message.occurred_at).toLocaleString()}</dd></dl>{message.direction === "inbound" ? <button type="button" onClick={() => onReply(message.reply_to_addresses[0] || message.from_address, message.subject)}>답장</button> : null}</header><pre className="mail-body">{message.text_body || message.html_body || "본문이 없습니다."}</pre>{message.attachments.length ? <section><h3>첨부파일</h3><ul>{message.attachments.map((attachment, index) => <li key={attachment.id || `${attachment.filename}-${index}`}>{attachment.filename || "첨부파일"} {attachment.size ? `· ${attachment.size.toLocaleString()} bytes` : ""}</li>)}</ul></section> : null}</article>;
}
