...
function statusLabel(value: string | undefined | null) { const status = String(value || "").toUpperCase(); if (["HEALTHY", "LIVE", "COMPLETED", "OK", "WITHIN_RANGE", "COMPLETE_BALANCED"].includes(status)) return "정상"; if (status === "PROCESSING") return "처리 중"; if (status === "OBSERVED") return "확인됨"; if (["UNAVAILABLE", "FAILED", "CRITICAL"].includes(status)) return "연결 안 됨"; if (["NO_DATA", "UNKNOWN"].includes(status)) return "데이터 없음"; if (status === "PARTIAL_DATA") return "부분 수집"; return "주의"; }
