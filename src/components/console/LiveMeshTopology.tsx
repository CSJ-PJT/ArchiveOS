    ...
    <div className="mesh-canvas-scroll"><div className="mesh-canvas">
      ...
      {nodes.map((node) => {
        ...
        return <button key={node.id} type="button" className={`mesh-node mesh-node-${node.id} ${nodeEvents.length ? "mesh-node-active" : ""}`} style={{ left: `${node.x}%`, top: `${node.y}%` }} onClick={(event) => openSelection({ type: "node", node }, event.currentTarget)}>
          ...
          <span className="mesh-node-metrics"><StatusBadge status={runtimeState?.runtimeStatus || runtimeState?.serviceStatus || "waiting"}>{runtimeLabel(runtimeState?.runtimeStatus || runtimeState?.serviceStatus)}</StatusBadge><em>{nodeEventMetric(nodeEvents.length, runtimeState)}</em></span>
...
function normalizeNode(value?: string | null) { const text = String(value || "").toLowerCase(); if (text.includes("market")) return "market"; if (text.includes("nexus") || text.includes("factory")) return "nexus"; if (text.includes("logis") || text.includes("logit")) return "logistics"; if (text.includes("ledger") || text.includes("transaction")) return "ledger"; if (text.includes("settle")) return "settlement"; return "archiveos"; }
...
function eventAge(value?: string | null) { if (!value) return "최근 수신 대기"; const seconds = Math.max(0, Math.floor((Date.now() - new Date(value).getTime()) / 1000)); if (seconds < 60) return `${seconds}초 전 수신`; if (seconds < 3_600) return `${Math.floor(seconds / 60)}분 전 수신`; if (seconds < 86_400) return `${Math.floor(seconds / 3_600)}시간 전 수신`; return `${Math.floor(seconds / 86_400)}일 전 수신`; }
