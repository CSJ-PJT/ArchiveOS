...
- `winget install --id Docker.DockerDesktop --exact --silent`는 설치 파일 다운로드와 hash 검증까지 성공했으나, 설치 관리자가 관리자 권한 상승을 요구한 뒤 exit code `4294967291`로 실패
- Docker installer log: `ProcessEnvironmentDetector`가 "Not run as admin, relaunching with UAC prompt"를 기록했고, UAC 관리자 재실행 단계에서 작업이 취소되어 설치가 중단됨
- 일부 사용자 디렉터리(`AppData\Local\Docker`, `AppData\Roaming\Docker`)는 생성되었지만 `docker.exe`, Docker Desktop 실행 파일, Windows service는 생성되지 않음
