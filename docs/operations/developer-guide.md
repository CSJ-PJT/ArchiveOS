...
Docker Desktop이 설치되어 있는데 `docker` 명령만 실패하면 현재 세션에 PATH를 추가한다.
...
관리자 권한이 아니면 `C:\ProgramData\chocolatey\lib\docker-desktop` 접근이 거부되어 설치가 실패한다. 이 경우 관리자 PowerShell을 새로 열어 설치해야 한다.
...
## 8. 현재 환경에서 Docker가 없을 때

Docker CLI가 없으면 다음 결과가 나온다.

- `docker --version`: 실패
- `docker compose version`: 실패
- `docker info`: 실패
- `docker compose config`: 실행 불가
- 실제 pgvector container E2E 검증: 실행 불가
